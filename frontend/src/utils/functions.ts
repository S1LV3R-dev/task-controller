...
export async function login(login_str: string, password: string) {
  return axios.post('http://localhost:8000/login', { login_str, password })
    .then(() => router.push('/'))
    .catch((error) => {
...
export async function register(username: string, email: string, password: string) {
  return axios.post('http://localhost:8000/register', { username, email, password })
    .then(() => router.push('/'))
    .catch((error) => {
...
export async function get_task_by_id(task_id: number) {
  return axios.get(`http://localhost:8000/tasks/${task_id}`)
    .then((response) => response.data);
}
...
export async function update_task(
  ...
  return axios.put(`http://localhost:8000/tasks/${task_id}`, {
      name: taskTitle,
...
export async function delete_task(task_id: number) {
  return axios.delete(`http://localhost:8000/tasks/${task_id}`)
    .then((response) => response.data);
}
...
export async function change_status(task_id: number, status: number) {
  return axios.patch(`http://localhost:8000/tasks/${task_id}?status=${status}`)
    .then((response) => response.data);
}
